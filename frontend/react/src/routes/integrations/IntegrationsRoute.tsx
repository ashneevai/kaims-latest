...
export default function IntegrationsRoute() {
  ...
  const observedStatuses = (applications.data || []).map((row) => String(row.status || "registered").toLowerCase());
  const onboardingJourney = [
    { label: "Connect", detail: "Register application context", complete: observedStatuses.length > 0 },
    { label: "Discover", detail: "Discover services and resources", complete: observedStatuses.some((status) => !["registered", "failed"].includes(status)) },
    { label: "Verify", detail: "Validate metrics and permissions", complete: observedStatuses.some((status) => ["validated", "dashboard_created", "active"].includes(status)) },
    { label: "Activate", detail: "Enable governed operations", complete: observedStatuses.some((status) => ["dashboard_created", "active"].includes(status)) },
  ];

  return <section className="grid single-col integration-launchpad">
    <article className="panel"><div className="panel-head"><div><span className="eyebrow">CONNECT · DISCOVER · VERIFY · ACTIVATE</span><h2>Integration Launchpad</h2><p>Build operational readiness as a guided journey. Each completed stage is derived from persisted application status.</p></div></div><ol className="execution-stepper">{onboardingJourney.map((step, index) => <li key={step.label} className={step.complete ? "is-complete" : index === onboardingJourney.findIndex((candidate) => !candidate.complete) ? "is-current" : ""}><span>{step.complete ? "✓" : index + 1}</span><div><strong>{step.label}</strong><small>{step.detail}</small></div></li>)}</ol></article>
